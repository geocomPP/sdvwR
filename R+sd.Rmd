...
the previously loaded borrough level adminsitrative boundaries. 
...
The most obvious issue with the point data from the perspective of a spatial join with the 
burrough data is that many of the points in the dataset are in fact located outside 
...
```{r}
lnd.stations@data[1:10,]
```



As with any spatial attribute data stored as an `sp` object, we can look at the attributes 
...
In this case there is not much of interest, so we will create an artificial 
field called "MICE", representing an imaginary dataset of the number of reported 
mouse sitings in each sightings. This is so that we can allocate quantitative 
data to the administrative zones from the points, a common task in spatial 
data analysis. 
