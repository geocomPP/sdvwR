...
```{r The Robinson Projection, message=FALSE}
...
```{r A Map of the Continents Using Default Colours}
...
```{r Black and White Population Map}
...
```{r World Map with Yellow Green Colour Brewer Palette}
...
```{r Different Colour Palettes with Bespoke Breaks}
library(classInt)
...
```{r Using Stamen's Toner basemap}
